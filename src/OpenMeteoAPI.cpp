...
WeatherForecast OpenMeteoAPI::getForecast(float latitude, float longitude) const {
  ...
  JsonArray sunrise_array = doc["daily"]["sunrise"].as<JsonArray>();
  if (sunrise_array.size() > 0) {
    forecast.sunrise = sunrise_array[0].as<String>().substring(11, 16);
  }

  JsonArray sunset_array = doc["daily"]["sunset"].as<JsonArray>();
  if (sunset_array.size() > 0) {
    forecast.sunset = sunset_array[0].as<String>().substring(11, 16);
  }
